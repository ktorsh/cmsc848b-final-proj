...
from tools import *
from noisetools import addNoise
...
def build_patch_shape(M, N, T, hP, ht, shape):
    ...
    def meshgridperiodic(M, N=1, T=1):
        (Y, X, Z) = np.meshgrid(np.min(np.arange(1, N) - 1, N + 1 - np.arange(1, N)), \
                                np.min(np.arange(1, M) - 1, M + 1 - np.arange(1, M)), \
                                np.min(np.arange(1, T) - 1, T + 1 - np.arange(1, T)))
        return (X, Y, Z)
...
def central_pix(img_sample, param):
    ...
    img_nse1 = addNoise(x, sigma = noise)
    img_nse2 = addNoise(x, sigma = noise)
        ...
        for l in range(-(p + 1), (p + 1)):
            for t in range(-(ptemp + 1), ptemp + 1):
                if patch_shape[np.mod(k, M) + 1, np.mod(l, N) + 1, np.mod(t, T) + 1] == 1:
                    patch_nse1[:, n] = np.reshape(np.roll(img_nse1, (k, l, t)), (M * N * T, 1))
                    patch_nse2[:, n] = np.reshape(np.roll(img_nse2, (k, l, t)), (M * N * T, 1))
                    n += 1
    
    d = criteria(patch_nse1, patch_nse2[np.random.permutation(M * N * T), :], noise)
    d = np.mean(d, 2)

    m_criteria = np.mean(d)
    s_criteria = np.std(d)
    return (m_criteria, s_criteria)
...
def nlmeans(ima_nse, noise, param):
    ...
    shape = getoptions(param, 'shape', 'triangle')
    blur = getoptions(param, 'blur', 1)
    tau = getoptions(param, 'tau', 1)
    block = getoptions(param, 'block', True)
    dejitter = getoptions(param, 'dejitter', False)
    m_criteria = getoptions(param, 'm_criteria', [])
    s_criteria = getoptions(param, 's_criteria', [])

    if m_criteria == [] or s_criteria == []:
        param.p = hP
        param.p = hP
        param.p_temp = ht
        param.shape = shape
    ...
    patch_shape = np.conj(fftn(patch_shape))
    ...
    for dx in range(-hW, hW):
        for dy in range(-hW, hW):
            for dz in range(-hT, hT):
                ...
                diff = np.real(ifftn(patch_shape * fftn(diff)))

                w = np.exp(-np.abs(diff - m_criteria) / (s_criteria * tau ** 2))

                if block:
                    w = np.real(ifftn(patch_size * patch_shape * ifftn(w)))
